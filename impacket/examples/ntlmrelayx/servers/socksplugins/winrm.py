...
#
# Description:
#   Socks Proxy for the WinRM Protocol
#
#  A simple SOCKS server that proxies a connection to relayed WinRM connections
#
# Author:
...
# writing a plugin:
PLUGIN_CLASS = "WinRMSocksRelay"
EOL = b'\r\n'

class WinRMSocksRelay(SocksRelay):
    PLUGIN_NAME = 'WinRM Socks Plugin'
    PLUGIN_SCHEME = 'WINRM'
        ...
        try:
            ...
            if self.username in self.activeRelays:
                ...
                if self.activeRelays[self.username]['inUse'] is True:
                    LOG.error('WinRM: Connection for %s@%s(%s) is being used at the moment!' % (
                        self.username, self.targetHost, self.targetPort))
                    return False
                else:
                    LOG.info('WinRM: Proxying client session for %s@%s(%s)' % (
                        self.username, self.targetHost, self.targetPort))
                    self.session = self.activeRelays[self.username]['protocolClient'].session
            else:
                LOG.error('WinRM: No session for %s@%s(%s) available' % (
                    self.username, self.targetHost, self.targetPort))
                return False
